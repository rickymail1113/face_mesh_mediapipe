    ...
    while cap.isOpened():
        ...
        if result.multi_face_landmarks:
            ...
            # 468 個臉部標記
                ...
                # 臉部 網格
                mp_drawing.draw_landmarks(image=image, landmark_list=i, connections=mp_face_mesh.FACEMESH_TESSELATION,
                                         landmark_drawing_spec=None,
                                         connection_drawing_spec=mp_drawing_style.get_default_face_mesh_tesselation_style())

                # 臉、眉、嘴 輪廓
                mp_drawing.draw_landmarks(image=image, landmark_list=i, connections=mp_face_mesh.FACEMESH_CONTOURS,
                                         landmark_drawing_spec=None,
                                         connection_drawing_spec=mp_drawing_style.get_default_face_mesh_tesselation_style())

                # 瞳孔
                mp_drawing.draw_landmarks(image=image, landmark_list=i, connections=mp_face_mesh.FACEMESH_IRISES,
                                         landmark_drawing_spec=None,
                                         connection_drawing_spec=mp_drawing_style.get_default_face_mesh_tesselation_style())
